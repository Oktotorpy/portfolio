...
<PublicShell contact={data.contact} roles={data.roles} workTypes={data.workTypes}>
  ...
  {#if selectedProject}
    ...
    <div class="popup-card" style={popupStyle} on:click|stopPropagation>
      ...
      {#if selectedProject.content_type && selectedProject.content_value}
        <div class="popup-media">
          {#if selectedProject.content_type === 'image'}
            <img src={selectedProject.content_value} alt={selectedProject.name} />
          {:else if selectedProject.content_type === 'video'}
            <video src={selectedProject.content_value} controls preload="metadata"></video>
          {:else if getYouTubeId(selectedProject.content_value)}
            <iframe src="https://www.youtube-nocookie.com/embed/{getYouTubeId(selectedProject.content_value)}"
      ...
      <div class="popup-body">
        ...
        <p class="popup-meta">{selectedProject.role?.name || ''}{#if selectedProject.role?.job?.name} · {selectedProject.role.job.name}{/if}</p>
        ...
        {#if selectedProject.skills?.length > 0}
          <div class="popup-skills">{#each selectedProject.skills as skill}<span class="popup-skill">{skill.name}</span>{/each}</div>
        {/if}
      </div>
    </div>
  {/if}
